...
sequenceDiagram
    ...
    Attacker->>App: Registers username: reportname'--
    App->>DB: INSERT INTO users (username) VALUES (?)  [parameterized — stored safely]
    Note over DB: The payload sits in the database,<br/>inert, as a harmless string
    AdminPanel->>DB: SELECT * FROM logs WHERE username = 'reportname'--' [built via concatenation]
    Note over AdminPanel,DB: The SAME value, read back later,<br/>now reaches a vulnerable query in a different code path
```
